...
export async function generatePracticeFromUrl(sourceUrl: string, req?: any) {
  ...
  let youtubeEmbedFallbackWarning: string | null = null
  if (useYouTubeEmbedFallback) {
    youtubeEmbedFallbackWarning = (
      await prepareYouTubeEmbedFallbackSource(sourceUrl, workDir)
    ).warning
  } else {
    try {
...
async function prepareYouTubeEmbedFallbackSource(
  sourceUrl: string,
  workDir: string,
) {
  try {
