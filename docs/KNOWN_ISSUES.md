...
- 공식 rule catalog의 `learnerActionLabel`, `easyReason`, `unsafeConfusions` 같은 교육용 확장 필드는 아직 P1 범위다. 현재는 기존 `action`, `do_not`, `why`, `caregiver`, `report_script`에서 구조화 데이터를 만든다.
