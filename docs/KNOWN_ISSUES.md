...
- URL 입력 자동 생성 경로는 이제 GPT-5.5가 strict JSON으로 장면 분할과 교육 문구를 작성하고, 로컬은 ffmpeg scene-cut 후보/0.01초 경계 보정 리포트/LRS 품질 게이트로 검증한다. 다만 아직 영상 프레임 OCR까지 결합한 완전 자동 제작 파이프라인은 아니다. 자막이 없거나 자동자막 오인식이 심한 영상은 학습 화면을 만들지 않고 실패시키는 것이 현재의 안전한 동작이다.
