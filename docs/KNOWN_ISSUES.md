...
- URL 입력 자동 생성 경로는 이제 긴 자막 블록을 먼저 18초 이하로 나누고 LRS 품질 게이트로 낮은 품질 결과를 차단하지만, 아직 영상 프레임 OCR/장면 전환 검출/LLM structured repair까지 결합한 완전 자동 제작 파이프라인은 아니다. 자막이 없거나 자동자막 오인식이 심한 영상은 학습 화면을 만들지 않고 실패시키는 것이 현재의 안전한 동작이다.
