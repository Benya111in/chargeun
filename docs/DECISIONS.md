...
- 영향: `/`는 서비스 소개와 `학습 체험하기` CTA만 노출한다. CTA는 `#/scenario/fire-grounded-flow`로 이동하고, 화재 완료 후 지진으로 이어진다. 마지막 연습 종료 시 설문 요청 모달을 보여 주며, 실제 구글폼 URL은 `VITE_SURVEY_FORM_URL` 환경변수로 주입한다.
