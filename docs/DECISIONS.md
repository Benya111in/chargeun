...
### D-064 계절 재난 확장은 transcript가 명확한 공식 영상부터 넣는다

- 이유: 화재/지진만으로는 서비스가 특정 사례 데모처럼 보인다. 하지만 새 주제를 무작정 많이 넣으면 장면 분할과 쉬운말 품질이 흔들리므로, 안전한TV에서 음성/자막 흐름을 확인할 수 있고 공식 행동요령과 연결하기 쉬운 영상부터 확장한다.
- 영향: 1차 확장 주제는 호우, 태풍, 폭염, 한파, 대설로 고정했다. 각 주제는 별도 rule id, official source, official chunk, 로컬 압축 demo asset을 갖고 기존 `StructuredLearningExplanation` 검증 경로를 통과해야 한다. 공개 GitHub Pages 기본 흐름에는 아직 넣지 않는다.

### D-065 원본 공공 영상은 로컬 연구 자산과 웹 데모 자산을 분리한다

- 이유: 원본 mp4를 그대로 Git에 넣으면 저장소가 커지고, 공개 배포 시 2차 이용 조건 검토가 불명확해진다. 새 계절 재난 주제는 아직 사용성/권리 검토 전이므로 기존 GitHub Pages 체험 링크와 분리해야 한다.
- 영향: 원본은 `data/eval/sources/seasonal/*`에 ignored local source로 두고, 로컬 preview용 압축본도 `apps/desktop-ui/public/demo-video/seasonal/*` ignored asset으로 둔다. `/local-seasonal`은 dev 환경에서만 접근한다. 실제 서비스 배포 전에는 안전한TV/공공기관 영상 사용 권한과 출처 표기 정책을 따로 확정한다.

### D-066 계절 재난도 한 장면 한 판단 원칙을 강제한다

- 이유: 호우, 태풍, 폭염, 한파, 대설 영상은 짧아 보여도 한 구간 안에 알림 확인, 가족 연락, 물가 회피, 설비 점검처럼 서로 다른 판단이 연속해서 나온다. 한 세그먼트에 두 가지 이상을 묶으면 학습자는 어느 행동을 어떤 장면에 연결해야 하는지 헷갈릴 수 있다.
- 영향: 로컬 계절 재난 시나리오는 4~5개 큰 장면이 아니라 7~12개 짧은 장면으로 나누고, 모든 로컬 계절 장면은 30초 이하로 유지한다. 시간 구간은 겹치거나 비지 않게 이어져야 하며, 공식 음성/자막의 핵심 명사는 narration coverage 테스트로 보존한다. 새 판단점이 기존 rule과 맞지 않으면 호우 후 신고, 대설 농장 관리처럼 별도 rule/chunk를 추가한다.

### D-076 URL 자동 생성 결과는 LRS 품질 게이트를 통과해야만 노출한다

- 이유: URL 입력 경로가 자막을 빠르게 받아 휴리스틱으로 바로 장면을 만들면, 지금까지 쌓은 기준인 긴 음성 분할, 한 장면 한 판단, 핵심 명사 보존, 쉬운말 금지어 제거가 자동 생성에 반영되지 않을 수 있다. 사용자가 직접 지적한 뒤 고치는 방식은 제품 품질 기준이 아니다.
- 영향: `/api/generate-practice-from-url`은 생성 직후 `url_generation_lrs_v1` 검사를 수행한다. 자막/오디오 주제 수보다 장면 수가 부족하거나, 한 장면이 30초를 넘거나, 행동/하지 말아요/확인 질문이 빠지거나, 학습자 화면 문구에 `유입`, `찾기해야`, `나리`, `채소` 같은 어려운 표현·자동자막 오인식이 남으면 결과를 저장하지 않고 실패시킨다. 통과한 결과에는 `generationQualityReport`를 함께 저장한다.

### D-077 알 수 없는 URL 영상은 차단 전에 bounded fallback 분할을 먼저 한다
